...
import { signInAsync } from './signupSlice';
...
const SignIn = () => {
  ...
  const SignupSchema = Yup.object().shape({
    ...
    email: Yup.string().email('Invalid email').required("Email is required")
      .test('Unique Email', 'Email not found',
        function (value) {
          return new Promise((resolve, reject) => {
            axios.get(`${BASE_URL}/api/Account/check-email?email=${value}`)
              .then((res) => {
                resolve(false)
              })
              .catch((error) => {
                    ...
                    <Form>
                      ...
                      <div className='mb-5'>
                        <button type='submit' className='w-full cursor-pointer rounded-lg border border-primary bg-primary p-4 text-white transition hover:bg-opacity-90'>
                          {status == "loading" ? <div className='flex justify-center w-full'>  <ReactLoading type='bars' height={24} width={24} /></div> : " Sign In"}
                          {/* Sign In */}
                        </button>
                      </div>
